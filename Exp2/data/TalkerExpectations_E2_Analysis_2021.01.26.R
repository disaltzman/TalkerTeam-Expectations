...
# Load Demographics ----
# Read in files
file_names <- list.files(path = "./questionnaire", pattern = "*.csv", all.files = FALSE,
                         full.names = TRUE, recursive = FALSE)
...
summarySE(df, measurevar = "correct") # overall accuracy

acc_summary_byGroup <- summarySE(df, measurevar = "correct", 
                                 groupvars = c("expectations")) # acc by group
...
# Limit RT analysis to correc trials
df_correct <- droplevels(subset(df, correct == 1))

# Mean RTs by subject
rt_summary <- Rmisc::summarySE(data = df_correct, measurevar = "reaction_time", 
...
rt_summary_byGroup <- Rmisc::summarySE(data = df_correct, measurevar = "reaction_time",
...
rt_model_slope_gamma <- mixed(reaction_time ~ condition * expectations + (condition||subject),
                              data=df_correct, family=Gamma(link="identity"), method="LRT", expand_re = TRUE,
                              control = glmerControl(optimizer="bobyqa", calc.derivs = FALSE,
                                                     optCtrl = list(maxfun = 1500000)))
...
rt_model_slope_invgauss <- mixed(reaction_time ~ condition * expectations + (condition||subject),
                                 data=df_correct, family=inverse.gaussian(link="identity"), method="LRT", expand_re = TRUE,
                                 control = glmerControl(optimizer="bobyqa", calc.derivs = FALSE, 
                                                        optCtrl = list(maxfun = 1500000)))
...
# Save workspace ----
save.image("exp2-results-2021.01.26.RData")
